...
**決定**: 3台カメラ（cam0, cam2, cam3）で冗長性を確保。角度が異なるカメラほど三角測量の精度向上に寄与する。

**現状**: 3台とも正常にキャリブレーション済み（reprojection_error: 0.13-0.38px）。cam3は上方見下ろし角度で最も広角、三角測量に有用。
...
### DEC-03-4: 歪み補正の無効化（暫定、要修正）

**決定**: 全カメラの歪み係数を[0,0,0,0,0]に設定し、歪み補正を無効化。

**理由**: cam2のdist_coeffs（k2=11.5, k3=-49.9）が極端すぎてundistort_point()のNewton-Raphson法が収束しないため。

**問題**: 全カメラが巻き添えで無効化されている。cam0（k1=0.22）とcam3（k1=0.26）は穏当な係数で補正可能。歪み補正なしでは2D観測（歪み座標）と射影行列（無歪み座標）が不一致となり、リプロジェクションエラーの主因になっている。

**状態**: 要修正。全カメラの歪み補正を有効化することが最優先課題。
