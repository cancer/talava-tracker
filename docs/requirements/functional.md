...
- 三角測量の3D座標をそのまま使用（スケール=1.0メートル単位）
...
- カメラ検出: calibration.jsonのカメラ定義に基づく

### FR-04-2: フレーム同期・送信

- 全カメラのフレームが揃ってからFrameSetとして束ねてTCP送信
- JPEG圧縮（品質: camera_server.toml `jpeg_quality`、デフォルト80）
- Mac→Win方向のTCP接続で送信
...
- Mac側で完結し、camera_server起動時にTCP経由でinference_serverに送信

### FR-05-2: ポーズキャリブレーション（inference_server側）

- 手動: コンソール入力`c`で5秒後に実行、またはcamera_serverから`TriggerPoseCalibration`メッセージ
