...
pub fn build_osc_message(index: i32, enable: bool, pose: &TrackerPose) -> OscMessage {
    OscMessage {
        addr: "/VMT/Room/Unity".to_string(),
        args: vec![
            OscType::Int(index),
            OscType::Int(if enable { 1 } else { 0 }),
...
impl VmtClient {
    ...
    /// トラッカーの位置・回転を送信
    pub fn send(&self, index: i32, enable: bool, pose: &TrackerPose) -> Result<()> {
        let msg = build_osc_message(index, enable, pose);
        let data = encode_osc_message(&msg)?;
        let sent = self.socket.send_to(&data, &self.target_addr)?;
        println!("[DEBUG] sent {} bytes to {}: {:02x?}", sent, self.target_addr, &data);
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_build_osc_message_address() {
        let pose = TrackerPose::identity();
        let msg = build_osc_message(0, true, &pose);
    ...
    fn test_build_osc_message_args() {
        ...
        let msg = build_osc_message(1, true, &pose);
    ...
    fn test_build_osc_message_disabled() {
        let pose = TrackerPose::identity();
        let msg = build_osc_message(0, false, &pose);
