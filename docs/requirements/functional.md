...
- 単眼モード: 2Dキーポイント → FOV補正 → パースペクティブ補正 → スケール変換 → VR空間座標
- 複眼モード（world_coords）: 三角測量の3D座標をそのまま使用（スケール=1.0メートル単位）
...
- CONFIDENCE_THRESHOLD = 0.3（複眼）/ 0.2（単眼）
- 信頼度が閾値未満のキーポイントは無効化
...
### FR-06-4: 画像境界チェック（単眼モード）

- 四肢・肩の正規化座標 ≤0.02 or ≥0.98 を無効化
- 腰のX境界のみチェック（Y下方がフレーム外でも許容）
